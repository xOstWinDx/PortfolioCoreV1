...
class RateCommentUseCase(AbstractUseCase):
    def __init__(
    ...
    async def __call__(
        ...
        async with self.posts:
            if mode == "like":
                return await self.posts.like_comment(
                    post_id=post_id, user_id=context.user_id, comment_id=comment_id
                )
            elif mode == "dislike":
                return await self.posts.dislike_comment(
                    post_id=post_id, user_id=context.user_id, comment_id=comment_id
                )
            else:
